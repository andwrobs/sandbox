...
import {
  ...
  type PortalMessage,
  PORTAL_TARGET_ID,
} from "@andwrobs/portal-sdk";
import { usePortalAppStore } from "@andwrobs/portal-sdk/react";
...
export function SidePanel() {
  ...
  React.useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      try {
        // Check if the message is a valid portal message
        const message = event.data;
        if (!message || !message.type || !message.sourceId) return;
        ...
        setMessageLog((prev) => [
          // Add new message at the beginning (top)
          {
            ...
            event: message,
            direction:
              message.sourceId === PORTAL_TARGET_ID ? "outgoing" : "incoming",
          },
          ...prev,
        ]);
      } catch (error) {
        console.error("Error processing message in side panel:", error);
      }
    };
  ...
  const getAppName = (appId: string) => {
    if (apps[appId]?.name) return apps[appId].name;
    if (appId === PORTAL_TARGET_ID) return "Portal";
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    // Attach event listeners to document to track mouse movement outside the panel
